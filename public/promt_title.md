...
- **Từ khóa trọng tâm (Keyword)**: "###keyword###"
- **Tiêu đề chính của bài viết (Title)**: "###title###"
- **Website Brand**: "###website###"
- **Website URL**: "###href###"
- **Thông tin công ty**:
  ###company###
- **Danh sách link hình ảnh** (đã có sẵn): Chèn ảnh vào nội dung hợp lý nhất (chọn ảnh phù hợp nhất với nội dung đó)
  ###image_urls###
...
3.  **Nội dung (content)**:
    ...
    - Dùng 2-4 thẻ tiêu đề `<h2>`, mỗi `<h2>` phải kèm 1-3 thẻ tiêu đề `<h3>` phù hợp.
    - Đoạn văn ngắn gọn, dễ đọc (dưới 5 dòng/đoạn).
    - Sử dụng bullet points (-), numbering (1.), bold (**text**) để làm nổi bật ý quan trọng.
    - **Chèn link**: Chèn liên kết trỏ về `###href###` với anchor text là `###website###` một cách tự nhiên nhất.
...
4.  **Image Prompts (image_prompts)**:
    - Ngôn ngữ: Tiếng Anh.
    - Số lượng: Tạo prompts cho các H2 trong nội dung.
    - Tối giản prompt để AI generate ảnh nhanh nhất có thể.
    - Style: Photorealistic, cinematic lighting, Full HD, detail, valid aspect ratio 16:9.

### Quy tắc định dạng (BẮT BUỘC TUÂN THỦ 100%):

- Field "content" CHỈ ĐƯỢC CHỨA Markdown thuần túy.
- TUYỆT ĐỐI KHÔNG dùng bất kỳ HTML tag nào trong "content": không <p>, không <img>, không <a>, không <strong>, không <br>.
- Link dùng [text](url) — Image dùng ![alt](url) — Bold dùng **text**
...
```json
{
  "seo_title": "...",
  "seo_description": "...",
  "content": "Nội dung bài viết format Markdown.\n\nSử dụng \\n\\n để tách các đoạn văn, giúp định dạng hiển thị chuẩn xác nhất...",
  "image_prompts": [

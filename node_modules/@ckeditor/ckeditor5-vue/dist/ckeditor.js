...
import { version, defineComponent, useModel, ref, watch, onMounted, markRaw, onBeforeUnmount, openBlock, createBlock, resolveDynamicComponent, mergeModels, computed, watchEffect, shallowReadonly, toValue } from "vue";
import { debounce } from "lodash-es";
import { createIntegrationUsageDataPlugin, isCKEditorFreeLicense, appendExtraPluginsToEditorConfig, getCKBaseBundleInstallationInfo, isSemanticVersion, isCKTestingVersion, destructureSemanticVersion, uid, loadCKEditorCloud } from "@ckeditor/ckeditor5-integrations-common";
...
function assignInitialDataToEditorConfig(config, data) {
  var _a;
...
function getInitialDataFromEditorConfig(config) {
  var _a, _b;
  if (isRootsMapConfigurationSupported()) {
    return (_b = (_a = config.roots) == null ? void 0 : _a.main) == null ? void 0 : _b.initialData;
  }
  return config.initialData;
}
function isRootsMapConfigurationSupported() {
  const bundleInfo = getCKBaseBundleInstallationInfo();
  if (!bundleInfo) {
    return false;
  }
  return !isSemanticVersion(bundleInfo.version) || isCKTestingVersion(bundleInfo.version) || destructureSemanticVersion(bundleInfo.version).major >= 48;
}
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose, emit: __emit }) {
    ...
    checkVersion();
    onMounted(() => {
      let editorConfig = appendAllIntegrationPluginsToConfig(
